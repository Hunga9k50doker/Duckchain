...
const { loadData, sleep, getRandomNumber, updateEnv } = require("./utils");
...
class DuckChainAPIClient {
  ...
  async claimFaucet(authorization, address, proxyUrl) {
    const axiosInstance = this.createAxiosInstance(proxyUrl);
    try {
      const response = await axiosInstance.get(`https://testnet-faucet-api.duckchain.io/api/duckchain/claim_faucet?address=${address}`, {
        headers: {
          ...this.headers,
          Authorization: authorization,
        },
  ...
  async runAccount() {
    ...
    if (userInfo.success) {
      ...
      if (settings.CLAIM_FAUCET) {
        if (!this.wallets[i]) {
          this.log("Không tìm thấy địa chỉ ví, bỏ qua...", "warning");
        } else {
          this.log("Đang claim ton testnet...", "info");
          await this.claimFaucet(authorization, this.wallets[i], currentProxy);
        }
      }
...
async function runWorker(workerData) {
  ...
  const to = new DuckChainAPIClient(queryId, accountIndex, proxy);
  try {
...
async function main() {
  ...
  queryIds.map((val, i) => new DuckChainAPIClient(val, i, proxies[i]).createUserAgent());
