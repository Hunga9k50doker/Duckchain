...
class DuckChainAPIClient {
  constructor() {
    ...
    this.wallets = loadData("wallets.txt");
    this.authFaucet = loadData("authFacet.txt");
  }
  ...
  async collectDailyEgg(authorization) {
    try {
      const checkResponse = await axios.get("https://preapi.duckchain.io/property/daily/isfinish?taskId=1", {
        headers: {
          ...this.headers,
          Authorization: `tma ${authorization}`,
        },
      ...
      if (checkResponse.data.code === 200) {
        if (checkResponse.data.data === 0) {
          const collectResponse = await axios.get("https://preapi.duckchain.io/property/daily/finish?taskId=1", {
            headers: {
              ...this.headers,
              Authorization: `tma ${authorization}`,
            },
    ...
    while (true) {
      for (let i = 0; i < data.length; i++) {
        ...
        if (userInfo.success) {
          ...
          // if (userInfo.data.particleWallet && settings.CLAIM_FAUCET) {
          //   const walletAddressIndex = this.wallets.findIndex((wallet) => wallet === userInfo.data.particleWallet);
          //   if (walletAddressIndex >= 0) {
          //     getOrCreateJSON(
          //       this.session_name,
          //       {
          //         wallet: userInfo.data.particleWallet,
          //         token: this.authFaucet[walletAddressIndex],
          //       },
          //       "wallets.json"
          //     );
          //     sleep(2);
          //     this.log("Đang claim ton testnet...", "info");
          //     await this.claimFaucet(this.authFaucet[walletAddressIndex], userInfo.data.particleWallet);
          //   }
          // }
